...
asyncio_lock = asyncio.Lock()  # Define the asyncio lock for synchronization
...
async def harem(update: Update, context: CallbackContext, page=0) -> None:
    ...
    async with asyncio_lock:  # Applying the lock to synchronize access
        ...
        keyboard = [[InlineKeyboardButton(f"一═デ︻ нαяєм ︻デ═一 ({total_count})", switch_inline_query_current_chat=f"collection.{user_id}")]]
...
application.add_handler(CommandHandler(["hharem", "dcollection"], harem,block=False))
